...
public class VehicleList extends ArrayList<Vehicle> implements IFunctionList{
            ...
            do {
                ...
                switch(choice){
                    case 1:// Search by vehicle Name (descending).
                        text1 = Utils.getString("Enter Vehicle Name that you want to search: ");
                        ArrayList<Vehicle> clonelist = (ArrayList<Vehicle>) list.clone();
                        Collections.sort(clonelist, new Comparator<Vehicle>() {
                            @Override
                            public int compare(Vehicle c1, Vehicle oc2) {
                                return (oc2.vehicleId.compareTo(c1.vehicleId));
                            }
                        });
                        ...
                        break;
                    case 2:
                        text2 = Utils.getString("Enter Vehicle ID that you want to search: ");
                        for(Vehicle vh : list){
                            if(vh.getName() != null && (vh.getVehicleId().equals(text2))){
